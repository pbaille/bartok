...
(defn abs-move 
  ([h-context h-function]
    (conj h-context {:current (transpose (:center h-context) (interval (:degree h-function)))
...
(defn rel-move 
  ([h-context h-function]
    (conj h-context {:current (transpose (:current h-context) (interval (:degree h-function)))
                     :h-function h-function}))
  ([hc hf d-class]
...
(defn set-center [h-context new-center]
  {:pre [(type= h-context 'HarmonicContext)]}
  (let [i  (interval (mother-root (b> new-center)) 
                     (mother-root (:current h-context)))
        d (if (< (:val i) 0) (relative (degree i)) (degree i))]
    (conj h-context 
      {:center (-> h-context :current mother-mode) 
