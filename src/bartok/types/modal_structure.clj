; (ns bartok.harmony.modal-structure
;   (:use utils.all)
;   (:use bartok.primitives)
;   (:require [clojure.string :as s]))

; (def ^:private known-ms-syns
;  (reduce-kv
...
;   ex: (modal-struct-class :69) => (:M2 :P5 :M3 :M6)
;       (modal-struct-class :m∆9) => (:M2 :P5 :M7 :m3)
;       (modal-struct-class :sus2) => (:M2 :P5)"
;   [kw]
;   (let [[_ base seventh extensions] (re-find ms-pat (name kw)) 
;         base (if base (known-ms-syns (keyword base)) [:M3 :P5]) ;defaults to Major
...
;   ex: (modal-struct :C69) => {:root :C, :degrees (:M2 :M3 :P5 :M6), :pitch-classes (:C :D :E :G :A)}
;       (modal-struct :Fm∆9) => {:root :F, :degrees (:M2 :m3 :P5 :M7), :pitch-classes (:F :G :Ab :C :E)}
;       (modal-struct :Absus2) => {:root :Ab, :degrees (:M2 :P5), :pitch-classes (:Ab :Bb :Eb)}
;       (modal-struct :C/B) => {:root :C, :degrees (:M7 :M3 :P5), :pitch-classes (:B :C :E :G), :bass :B}
;       (modal-struct :C|B) => {:root :B, :degrees (:m2 :M3 :P4 :P5 :m6), :pitch-classes (:B :C :D# :E :F# :G), :bass :B}
;       (modal-struct :C|B/D#) => {:root :B, :degrees (:m2 :M3 :P4 :P5 :m6), :pitch-classes (:B :C :D# :E :F# :G), :bass :D#}"
;   [x]
...
;         root            (keyword root)
;         degrees         (modal-struct-class (or class "Maj"))
